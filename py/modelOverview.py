...
def main():
    ...
    for f in os.listdir("cvmodels"):
        if re.match(".*\.pickle$", f) and f != "bestModel.pickle":
            ...
            if conf >= 0:
                cvmodels.append((name, conf, deg, f"cvmodels/{f}", cv))
    # cvmodels.sort(key=lambda x: -x[2])
    df = pds.DataFrame(cvmodels, columns=["name", "conf", "degree", "path", "cv"])
    ...
    with pds.option_context('display.max_rows', None,
        ...
        # print(df)
        m = df.groupby(["degree", "name"])["conf"].aggregate(scistats.mstats.gmean)
        # m.sort_values(ascending=False, inplace=True)
        # print(m)
        df = df.join(m, on=["degree", "name"], rsuffix="_m")
        df.sort_values(by=["conf_m"], ascending=False, inplace=True)
        ...
        # print(tabulate([[m[3] if len(m) >= 4 else None, m[4], m[2]] for m in cvmodels], headers=["degree", "name", "conf"], missingval='N/A', showindex=True))
