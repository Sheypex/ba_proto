...
import math
import re
from typing import (List, Optional, NamedTuple, Tuple, Any, Dict, Union, Callable, Iterable, Sequence)
...
def printStat(txt: str, stat: str = ":information:", indent: int = 0) -> None:
	if indent > 0:
		ind = ["  "] * indent
		ind = "".join(ind)
	else:
...
def printSucc(txt: str, indent: int = 0) -> None:
	printStat(txt, ":white_check_mark:", indent)


def printInfo(txt: str, indent: int = 0) -> None:
	printStat(txt, ":information:", indent)


def printError(txt: str, indent: int = 0) -> None:
	printStat(txt, ":stop_sign:", indent)


def printWarn(txt: str, indent: int = 0) -> None:
	printStat(txt, ":warning:", indent)
...
def getTransformers(X: pds.DataFrame, polyDeg: int) -> Tuple[
	...
	if polyDeg > 1:
		poly: preprocessing.PolynomialFeatures = preprocessing.PolynomialFeatures(degree=polyDeg, interaction_only=True).fit(X)
	else:
		poly: preprocessing.PolynomialFeatures = preprocessing.PolynomialFeatures(degree=polyDeg, interaction_only=True, include_bias=False).fit(X)
...
def applyTransformers(X: pds.DataFrame, scale: preprocessing.StandardScaler, poly: preprocessing.PolynomialFeatures, scale2: preprocessing.StandardScaler) -> List[List[float]]:
...
def getSplits(dF: pds.DataFrame, polyDeg: int, x_cols: Union[str, List[str]], y_cols: Union[str, List[str]], cvSize: int = 0, unknownSize: int = 0, wfs: Optional[List[str]] = None,
              randomOrder: bool = True) -> Iterable[TrainTestUnknownSplit]:
	assert 1 <= polyDeg <= 5
	assert 0 <= cvSize <= 4
	...
	splits = list()
	#
	unknownCombs: List[Tuple[List[str], List[str]]] = [(list(ukwfs), [wf for wf in wfs if wf not in ukwfs]) for ukwfs in itertools.combinations(wfs, unknownSize)]
	for ukSplit in unknownCombs:
		ukwfs, kwfs = ukSplit
		#
		cvCombs: List[Tuple[List[str], List[str]]] = [(list(cvwfs), [wf for wf in kwfs if wf not in cvwfs]) for cvwfs in itertools.combinations(kwfs, cvSize)]
...
def main():
	...
	argp.add_argument("models", action="store", nargs="*", default="all", choices=["all"] + get_model_names() + [f"_{x}" for x in get_model_names()])
	...
	if type(cliArgs.models) is str:  # in case only one model or 'all' is given make sure it's still in the format of a list
	...
	if p.exists() and not p.is_dir():
		raise FileExistsError(f"The path {p} exists but is not a directory. It is an invalid location for storing models.")
	assert (cliArgs.cvSize + cliArgs.unknownSize <= 4), \
	...
	# print(dF)
	all_x_cols = ["build-linux-kernel1", "fio2", "fio3", "fio4", "fio5", "fio6", "fio7", "fio8", "fio9", "iperf10", "iperf11", "iperf12", "iperf13", "john-the-ripper14", "john-the-ripper15",
	              "ramspeed16", "ramspeed17", "ramspeed18", "ramspeed19", "ramspeed20", "ramspeed21", "ramspeed22", "ramspeed23", "ramspeed24", "ramspeed25", "stream26", "stream27", "stream28",
	              "stream29", "pCpu", "cpus", "rss", "vmem", "rchar", "wchar", "syscr", "syscw"]
	...
	#
	wfs = dF.wfName.unique()
	wfShortnamesLUT = {wf: re.compile(r"nfcore/(\w+):.*").match(wf).group(1) for wf in wfs}
	...
	with commons.stdProgress(rc) as prog:
		totalProg = prog.add_task("Total", total=cliArgs.numRepeats * numSplits * len(get_models(restrict=cliArgs.models)))
		...
		for iReps in range(cliArgs.numRepeats):
			...
			for split in splits:
				...
				if len(ukwfs) > 0:
					...
					uksName = "None"
				doModels = fit_models(X_train, y_train, X_test, y_test, X_unknown, y_unknown, X_full, y_full, doDegree, picklePrefix=f"{picklePrefixes[doDegree]}Model.", randomOrder=True,
				                      notRecompute=not cliArgs.recompute, maxiter=cliArgs.maxiter, onlyImprove=cliArgs.improve, modelsToProduce=cliArgs.models, showDone=cliArgs.showDone,
				                      cvPostfix=f"_CV-{cvsName}_U-{uksName}", modelsPath=cliArgs.modelsPath, sanityCheck=cliArgs.sanityCheck, saveBest=cliArgs.saveBest,
...
def showResults(models, degree, cvSize=0, cvSummary=False, latex=False):
		...
		for j, g in enumerate(grouped):
			...
			for i, d in enumerate(g):
				...
				if i == math.floor(len(g) / 2):
					g[i] = [p, name, test_conf, f"{avg:.4f}" if avg is not None else None, cv]
				else:
					g[i] = [p, name, test_conf, "", cv]
			grouped[j] = g
		grouped.sort(
			key=lambda x: float(x[math.floor(len(g) / 2)][3]) if x[math.floor(len(g) / 2)][3] is not None else float(
				"-inf"), reverse=True)
	...
	table = rich.table.Table(*tableHeaders)
	for row in data:
		table.add_row(*row)
	print(table)
...
class ScistatsNormBetween:
	def __init__(self, small: float, large: float, cond: Optional[Callable[[float], bool]] = None, div: Optional[float] = 2, toint: Optional[bool] = False,
	             clip: Optional[Union[bool, Sequence]] = False, hardClip: Optional[Union[bool, Sequence]] = False, center: Optional[float] = None) -> None:
		if small <= large:
			self.lower = small
			self.upper = large
		else:
...
class ScistatsNormAround(ScistatsNormBetween):
	def __init__(self, center: float, dist: float, cond: Optional[Callable[[float], bool]] = None, div: Optional[float] = 2, toint: Optional[bool] = False, clip: Optional[bool] = False,
	             hardClip: Optional[bool] = False) -> None:
...
class SciStatsNormBetweenRandTuple:
	def __init__(self, small: float, large: float, tupleSize: Tuple, cond: Optional[Callable[[float], bool]] = None, div: Optional[float] = 2, toint: Optional[bool] = False,
	             clip: Optional[Union[bool, Sequence]] = False, hardClip: Optional[Union[bool, Sequence]] = False, center: Optional[float] = None, maxTotal: Optional[float] = None):
...
def get_models(randomOrder: bool = False, maxiter: int = -1, restrict: Optional[List[str]] = None) -> List[
	...
	models: List[Tuple[str, Any, Union[None, Dict[str, Any]], str, Union[None, Dict[str, Any]]]] = [
		...
		("SVR-linear", svm.SVR(max_iter=maxiter, cache_size=1000),
		 {"kernel" : ["linear"], "C": ScistatsNormBetween(0, 1e2, cond=(lambda x: x > 0)),
		  "epsilon": ScistatsNormBetween(0, 10, cond=(lambda x: x >= 0.1)),
		  "tol"    : ScistatsNormAround(0, 1e-1, cond=(lambda x: x >= 1e-3), clip=True), }, "SVR - linear", None),
		("SVR-poly", svm.SVR(max_iter=maxiter, cache_size=1000),
		 {"kernel" : ["poly"], "degree": [2, 3],
		  ...
		  "coef0"  : ScistatsNormAround(0, 10), }, "SVR - poly", None),
		("SVR-rbf", svm.SVR(max_iter=maxiter, cache_size=1000),
		 {"kernel" : ["rbf"], "gamma": ScistatsNormBetween(0, 1, cond=(lambda x: x > 0)),
		  "C"      : ScistatsNormBetween(0, 1e2, cond=(lambda x: x > 0)),
		  "epsilon": ScistatsNormBetween(0, 10, cond=(lambda x: x >= 0.1)),
		  "tol"    : ScistatsNormAround(0, 1e-1, cond=(lambda x: x >= 1e-3), clip=True), }, "SVR - rbf", None),
		("SVR-sigmoid", svm.SVR(max_iter=maxiter, cache_size=1000),
		 {"kernel" : ["sigmoid"], "gamma": ScistatsNormBetween(0, 1, cond=(lambda x: x > 0)),
		  "C"      : ScistatsNormBetween(0, 1e2, cond=(lambda x: x > 0)),
		  "epsilon": ScistatsNormBetween(0, 10, cond=(lambda x: x >= 0.1)),
		  "tol"    : ScistatsNormAround(0, 1e-1, cond=(lambda x: x >= 1e-3), clip=True),
		...
		("LassoCV", linear_model.LassoCV(max_iter=maxiterPos, n_jobs=-1),
		 {"eps"      : ScistatsNormBetween(1e-4, 1e-2, cond=(lambda x: x > 0)),
		  ...
		  # 'cyclic']
		  # 'warm_start': [True, False]
		  }, "Lasso CV", {"skipPreElim": True, "cv": 1}),
		("Ridge", linear_model.Ridge(), {"alpha": ScistatsNormAround(1, 10, cond=(lambda x: x > 0)),
		                                 "tol"  : ScistatsNormAround(0, 1e-1, cond=(lambda x: x >= 1e-3), clip=True), },
		 "Ridge", None),
		("RidgeCV", linear_model.RidgeCV(alphas=ScistatsNormAround(1, 100, cond=(lambda x: x > 0)).rvs(size=5)),
		 {"cv": ScistatsNormBetween(2, 4, clip=True, toint=True)}, "Ridge CV",
		...
		("ElasticNetCV", linear_model.ElasticNetCV(max_iter=maxiterPos, n_jobs=-1),
		 {"l1_ratio": ScistatsNormBetween(0, 1, clip=True),
		  ...
		  "n_alphas": ScistatsNormBetween(10, 1000, cond=(lambda x: x >= 10), toint=True),
		  "tol"     : ScistatsNormAround(0, 1e-2, cond=(lambda x: x >= 1e-4), clip=True),
		  "cv"      : ScistatsNormBetween(2, 4, clip=True, toint=True), "selection": ["random"]  # , 'cyclic']
		  # 'warm_start': [True, False]
		  }, "Elastic Net CV", {"skipPreElim": True, "cv": 1}), ("BayesianRidge", linear_model.BayesianRidge(), {
			...
			"tol"        : ScistatsNormAround(0, 1e-1, cond=(lambda x: x >= 1e-3), clip=True), }, "Bayesian Ridge", None),
		...
		("SGD", linear_model.SGDRegressor(max_iter=maxiterPos),
		 {"loss"         : ["squared_loss", "huber", "epsilon_insensitive", "squared_epsilon_insensitive"], "penalty": ["l2", "l1", "elasticnet"],
		  ...
		  # for loss=huber, epsilon_insensitive,
		  # squared_epsilon_insensitive
		  "learning_rate": ["constant", "optimal", "invscaling", "adaptive"],
		  "eta0"         : ScistatsNormBetween(1e-3, 1e-1, cond=(lambda x: x > 0)),
		  "tol"          : ScistatsNormAround(0, 1e-1, cond=(lambda x: x >= 1e-3), clip=True),
		  # for learning_rate=constant, invscaling,
		  # adaptive
		  "power_t"      : ScistatsNormBetween(0, 1, cond=(lambda x: 0 < x < 1)),
		...
		("PA", linear_model.PassiveAggressiveRegressor(max_iter=maxiterPos),
		 {"C"      : ScistatsNormAround(0, 1e3, cond=(lambda x: x > 0)),
		  ...
		  "epsilon": ScistatsNormAround(0.1, 10, cond=(lambda x: x >= 1e-2))  # 'warm_start': [True, False]
		...
		("TheilSen", linear_model.TheilSenRegressor(n_jobs=-1, max_iter=maxiterPos, tol=ScistatsNormAround(0, 1e-1, cond=(lambda x: x >= 1e-3), clip=True).rvs()), None, "Theil Sen", None),
		("NNR", neural_network.MLPRegressor(max_iter=maxiterPos),
		 {"hidden_layer_sizes": SciStatsNormBetweenRandTuple(50, 150, (1, 3), clip=True, center=100, toint=True, maxTotal=300),
		  ...
		  "tol"               : ScistatsNormBetween(0, 1e-2, clip=True, cond=lambda x: x >= 1e-5), "warm_start": [True, False],
		  "early_stopping"    : [True],  # False
		  "power_t"           : ScistatsNormBetween(0, 1, hardClip=True, div=3),
		  "momentum"          : ScistatsNormBetween(0.5, 1, hardClip=True, center=0.9),
		  "nesterovs_momentum": [True, False], "beta_1": ScistatsNormBetween(0.8, 1, hardClip=True, center=0.9),
		  "beta_2"            : ScistatsNormBetween(0.9, 1, hardClip=True, center=0.999), }, "Neural Network Regressor",
		 {"minBaseRes": 40, "minCand": 100, "cv": 1, "maxNumTurns": 10, "maxPrelim": 3}),
		("NNC", neural_network.MLPClassifier(max_iter=maxiterPos), {
			"hidden_layer_sizes": SciStatsNormBetweenRandTuple(50, 150, (1, 3), clip=True, center=100, toint=True, maxTotal=300),
			"activation"        : ["identity", "logistic", "tanh", "relu"],
			"solver"            : ["lbfgs", "sgd", "adam"], "alpha": ScistatsNormBetween(10e-7, 10, clip=True, center=1e-1),
			"learning_rate"     : ["constant", "invscaling", "adaptive"],
			"learning_rate_init": ScistatsNormBetween(1e-5, 1e-2, clip=True, center=1e-3),
			"tol"               : ScistatsNormBetween(0, 1e-2, clip=True, cond=lambda x: x >= 1e-5),
			"warm_start"        : [True, False], "early_stopping": [True],  # False
			"power_t"           : ScistatsNormBetween(0, 1, hardClip=True, div=3),
			"momentum"          : ScistatsNormBetween(0.5, 1, hardClip=True, center=0.9), "nesterovs_momentum": [True, False],
			"beta_1"            : ScistatsNormBetween(0.8, 1, hardClip=True, center=0.9),
			"beta_2"            : ScistatsNormBetween(0.9, 1, hardClip=True, center=0.999), }, "Neural Network Classifier",
		 {"minBaseRes": 40, "minCand": 100, "cv": 1, "maxNumTurns": 10, "maxPrelim": 3}),
		...
		("DTC", tree.DecisionTreeClassifier(),
		 {"criterion": ["gini", "entropy"], "splitter": ["best", "random"], "max_features": ["auto", "sqrt", "log2"], },
...
def _getHRSCVTournamentParams(params, halvingParams, X_train):
	...
	baseRes = ScistatsNormBetween(minBaseRes, maxBaseRes if maxBaseRes >= minBaseRes else minBaseRes, cond=(lambda x: x > 2 * cv * regrcv.upper), toint=True).rvs()
	# TODO: x>=9 is kind of arbitrary, whenever the regr also does CV, x must be bigger than 2*cv*<cv of regr> --> for x>=9 this should be the case
	# but not all models
	# require x>=9 so its a dirty fix for now
	numTurns = ScistatsNormBetween(minNumTurns, maxNumTurns, clip=True, toint=True, center=minNumTurns + 1 / 3 * (maxNumTurns - minNumTurns)).rvs()
...
def sanity_check(pFile: Union[Path, str], X_train: List[List[float]], y_train: List[List[float]], X_test: List[List[float]], y_test: List[List[float]], X_unknown: List[List[float]],
                 y_unknown: List[List[float]], X_full: List[List[float]], y_full: List[List[float]], bonusPickleInfo: Optional[Any] = None, loaded: Optional[PickleOut] = None) -> PickleOut:
	if loaded is None:
		loaded: PickleOut = pickle.load(open(pFile, "br"))
	try:
		(longname, regr, test_confidence, deg, full_confidence, unknown_confidence, train_confidence,
		 bonusPickleInfo) = loaded
	except:
	...
	toDump = PickleOut(longname, regr, test_confidence, deg, full_confidence, unknown_confidence, train_confidence,
	                   bonusPickleInfo)
...
def fit_models(X_train: List[List[float]], y_train: List[List[float]], X_test: List[List[float]], y_test: List[List[float]], X_unknown: List[List[float]], y_unknown: List[List[float]],
               X_full: List[List[float]], y_full: List[List[float]], polyDeg: int, models: Optional[List[Tuple[str, Any, Union[None, Dict[str, Any]], str, Union[None, Dict[str, Any]]]]] = None,
               picklePrefix: Optional[str] = "", randomOrder: Optional[bool] = False, notRecompute: Optional[bool] = True, maxiter: Optional[int] = -1, onlyImprove: Optional[bool] = False,
               modelsToProduce: Optional[List[str]] = None, showDone: Optional[bool] = False, cvPostfix: Optional[str] = None, modelsPath: Union[Path, str] = None, sanityCheck: Optional[bool] = False,
               saveBest: Optional[bool] = False, bonusPickleInfo: Optional[Any] = None,
               progressBar: Optional[Union[Tuple[rich.progress.Progress, int], List[Tuple[rich.progress.Progress, int]]]] = None) -> List[PickleOut]:
	...
	def custom_scoring(est: sklearn.base.RegressorMixin, X: List[List[float]], y: List[List[float]]) -> float:
		...
		test_confidence = est.score([t for i, t in enumerate(X_test) if i in test_samples],
		                            [t for i, t in enumerate(y_test) if i in test_samples])
		full_confidence = est.score([t for i, t in enumerate(X_full) if i in full_samples],
	...
	if models is None:
		models: list[tuple[str, Any, Optional[dict[str, Any]], str, Optional[dict[str, Any]]]] = get_models(randomOrder=randomOrder, maxiter=maxiter, restrict=modelsToProduce)
	...
	for model in models:
		...
		if params is not None:
			...
			searchParams = {"estimator"             : regr,
			                ...
			                "error_score"           : -1e-50,
			                "cv"                    : cv, "verbose": 1, }
			regr = HalvingRandomSearchCV(**searchParams)  # regr = RandomizedSearchCV(estimator=regr, param_distributions=params, n_iter=2 ** 8, n_jobs=-1, refit=True,  #
		# return_train_score=True,
		# verbose=2, cv=10)
			...
			try:
				...
				if onlyImprove:
					...
					if not pFile.is_file():
						...
						table = rich.table.Table("", "test_conf", "full_conf", "train_conf", "geo_mean")
						table.add_row("old:", str(loaded.test_confidence), str(loaded.full_confidence), str(loaded.train_confidence), str(loadedGeo))
						table.add_row("new:", str(test_confidence), str(full_confidence), str(train_confidence), str(newGeo))
						geoDiff = newGeo - loadedGeo
						geoPercent = (100 * newGeo / loadedGeo) - 100
						if newGeo >= loadedGeo:
							pickle.dump(toDump, open(pFile, "bw"))
							printInfo(
								f"Improved (or matched) {fullname} (over {pickleName}.pickle): (+{geoDiff:.5f}, +{geoPercent:.5f}%)", 1)
							trained.append(toDump)
						else:
							printInfo(
								f"Did not improve {fullname} (over {pickleName}.pickle): ({geoDiff:.5f}, {geoPercent:.5f}%)", 1)
							trained.append(loaded)
						print("\n".join(["\t" + row for row in repr(table).split("\n")]))
				else:
					pickle.dump(toDump, open(pFile, "bw"))
